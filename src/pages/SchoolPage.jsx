...
export function SchoolPage() {
  ...
  const done = state.profile.completedLessons || []

  const complete = (id) => updateProfile({ completedLessons: [...new Set([...done, id])] })

  return (
    <>
      <RouteNav items={learningNav} />
      <PageHeader eyebrow="Aprender" title="Escola AVYO" subtitle="Lições curtas para tomar decisões com mais calma e contexto." />
      <div className="mb-5 flex items-center justify-between gap-3 rounded-xl bg-white/[0.025] px-4 py-3 text-sm text-slate-400">
        <span>Progresso local</span><strong className="text-cyan-200">{done.length} de {lessons.length} concluídas</strong>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {lessons.map((lesson) => {
          const completed = done.includes(lesson.id)
          return (
            <button key={lesson.id} type="button" onClick={() => setActive(lesson)} className="text-left" aria-label={`${lesson.title} · ${completed ? 'Concluída' : 'Começar'}`}>
              <Card className="group h-full p-5 transition hover:-translate-y-0.5 hover:border-white/10">
                <div className="flex items-start gap-4">
                  <div className={`grid size-11 place-items-center rounded-xl ${completed ? 'bg-emerald-400/10 text-emerald-300' : 'bg-violet-400/10 text-violet-300'}`}>{completed ? <Check size={18} /> : <BookOpen size={18} />}</div>
                  <div><h2 className="font-heading text-lg font-semibold">{lesson.title}</h2><p className="mt-1 text-xs text-slate-500">{lesson.minutes} minutos · {completed ? 'Concluída' : 'Começar'}</p></div>
                </div>
              </Card>
            </button>
          )
      ...
      <LessonDialog lesson={active} onClose={() => setActive(null)} onComplete={complete} />
    </>
  )
}
